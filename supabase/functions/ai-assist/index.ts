...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // العرض التقديمي يُكتب بلغة واجهة المستخدم (يطابق لغة التطبيق للمستخدم الأجنبي)
    const SLIDE_LANG: Record<string, string> = {
      ar: "العربية",
      en: "English only (all titles and bullets in English)",
      fr: "le français uniquement (tous les titres et points en français)",
    };
    const systemPrompt =
      action === "translate"
        ...
        : action === "slides"
        ? `${SYSTEM.slides}\n\nمهم جدًا: اكتب كل نصوص الشرائح (العناوين والنقاط) بـ${SLIDE_LANG[targetLang] ?? SLIDE_LANG.ar} لتطابق لغة واجهة المستخدم، مهما كانت لغة النص الأصلي.`
        : SYSTEM[action];
